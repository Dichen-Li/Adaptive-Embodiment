...
class EfficientH5Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Efficiently load only the required slice of data
        with h5py.File(file_path, 'r') as f:  # Use SWMR for thread safety
            batch_idx, sample_idx = divmod(local_idx, f[self.dataset_key].shape[1])
            data = f[self.dataset_key][batch_idx, sample_idx]  # Load only the specific slice
...
if __name__ == "__main__":
    ...
    dataloader = DataLoader(dataset, batch_size=batch_size, num_workers=32, shuffle=False, drop_last=True)
