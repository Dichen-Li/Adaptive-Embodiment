...
log_dir="/tmu-fast-vol/code/jobs_play_collect_data_logs"
...
for task in "${tasks[@]}"; do
  ...
  cmd="/workspace/isaaclab/isaaclab.sh -p scripts/rsl_rl/play_collect_data.py --task \"$task\" --steps 600 --reward_log_file reward_log_file.json --min_ckpt_iter 14999 --headless ${kwargs[@]}"
  ...
  stdbuf -oL -eL /workspace/isaaclab/isaaclab.sh -p scripts/rsl_rl/play_collect_data.py --task "$task" --steps 600 --reward_log_file reward_log_file.json --min_ckpt_iter 14999 --headless "${kwargs[@]}" > "$log_file" 2>&1 || stop_execution
