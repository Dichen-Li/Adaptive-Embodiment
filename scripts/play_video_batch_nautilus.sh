...
if [[ "$#" -lt 2 ]]; then
  echo "Usage: bash play_video_batch_nautilus.sh --tasks task1 task2 ... [additional arguments]"
  exit 1
fi
...
log_dir="/dichen-fast-vol/code/jobs_play_video_logs"
...
for task in "${tasks[@]}"; do
  ...
  cmd="/workspace/isaaclab/isaaclab.sh -p scripts/rsl_rl/play.py --task \"$task\" --video --video_length 200 --headless ${kwargs[@]}"
  ...
  stdbuf -oL -eL /workspace/isaaclab/isaaclab.sh -p scripts/rsl_rl/play.py --task "$task" --video --video_length 200 --headless "${kwargs[@]}" > "$log_file" 2>&1 || stop_execution
