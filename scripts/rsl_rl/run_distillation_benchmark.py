...
def parse_arguments():
    ...
    parser.add_argument("--lr", type=float, default=1.25e-4, help="unit learning rate (for a batch size of 512)")
...
def train(policy, criterion, optimizer, scheduler, train_loader, val_loader, num_epochs, model_device,
    ...
    for epoch in range(num_epochs):
        ...
        data_loader_time_meter = AverageMeter()
        ...
        with tqdm.tqdm(train_loader, desc=f"Training Epoch {epoch + 1}/{num_epochs}", unit="batch") as pbar:
            for i in range(100000):
                ...
                data_loader_time_meter.update(time_taken)
                if i % 1000 == 0:
                    print(f"time for loading data: {data_loader_time_meter.avg:.5f}s")
            ...
            # for batch_inputs, batch_targets in pbar:
                ...
                if i % 1000 == 0:
                    print(f'time for training: {train_time_meter.avg:.5f}')
...
# def train(policy, criterion, optimizer, scheduler, train_loader, val_loader, num_epochs, model_device,
#           log_dir, checkpoint_interval, model):
#     """Training loop with I/O, memory, data loading, and computation profiling."""
#     train_loss_meter = AverageMeter()
#     best_val_loss = float("inf")
#
#     print("[INFO] Starting supervised training with profiling.")
#
#     # Enable profiler
#     with profile(
...
#             print(f"[INFO] Starting epoch {epoch + 1}/{num_epochs} - Training.")
#
#             # Profile data loading and training
...
#                 train_loss_meter.update(loss.item(), n=batch_targets.size(0))
#
#                 # Step profiler to capture the iteration
#                 prof.step()
#
#         # Print profiler results
#         print(prof.key_averages().table(sort_by="cuda_time_total", row_limit=20))
#
#     print("[INFO] Training completed with profiling results above.")
#
